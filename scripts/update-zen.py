...
if __name__ == "__main__":
    ...
    version = sys.argv[2]
    zen_file = sys.argv[3]
    pattern = re.compile(fr"v({version}\.?\d*)-({variant}\d+)")
    isFinded = False
    page = 1
    while not isFinded:
        zen_tags = github_api_request(f'repos/zen-kernel/zen-kernel/releases?page={page}&per_page=50')
        for tag in zen_tags:
            ...
            if zen_match:
                ...
                break
        page += 1
    old_version, old_suffix = read_file(zen_file, variant)
    if old_version != zen_version or old_suffix != zen_suffix:
        ...
        update_file(zen_file, variant, zen_version, zen_suffix, zen_hash)
